...
class FederationRuntimePlugin {
    ...
    static getFilePath(containerName, runtimePlugins, bundlerRuntimePath, experiments) {
        const hash = (0, utils_1.createHash)(`${containerName} ${FederationRuntimePlugin.getTemplate(runtimePlugins, bundlerRuntimePath, experiments)}`);
    ...
    getFilePath() {
        ...
        if (!this.options?.virtualRuntimeEntry) {
            this.entryFilePath = FederationRuntimePlugin.getFilePath(this.options.name, this.options.runtimePlugins, this.bundlerRuntimePath, this.options.experiments);
        }
        else {
            this.entryFilePath = `data:text/javascript;charset=utf-8;base64,${(0, btoa_1.default)(FederationRuntimePlugin.getTemplate(this.options.runtimePlugins, this.bundlerRuntimePath, this.options.experiments))}`;
        }
        return this.entryFilePath;
    }
    ensureFile() {
        ...
        try {
            ...
            fs_1.default.writeFileSync(filePath, FederationRuntimePlugin.getTemplate(this.options.runtimePlugins, this.bundlerRuntimePath, this.options.experiments));
        }
    }
    ...
    injectRuntime(compiler) {
        ...
        compiler.hooks.thisCompilation.tap(this.constructor.name, (compilation, { normalModuleFactory }) => {
            const isEnabledForChunk = (chunk) => {
                const [entryModule] = compilation.chunkGraph.getChunkEntryModulesIterable(chunk) || [];
                return entryModule instanceof ContainerEntryModule_1.default;
            };
    ...
    apply(compiler) {
        ...
        if (this.options?.experiments?.federationRuntime === 'hoisted') {
            ...
            new EmbedFederationRuntimePlugin_1.default(this.bundlerRuntimePath).apply(compiler);
            new HoistContainerReferencesPlugin_1.default(this.options.name ? this.options.name + '_partial' : undefined, 
            // hoist all modules of federation entry
            this.getFilePath(), this.bundlerRuntimePath).apply(compiler);
